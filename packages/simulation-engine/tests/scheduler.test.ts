...
describe('simulation-engine: scheduler and grid utilities', () => {
  ...
  it('should copy grid into newGrid before updates', () => {
    ...
    const priorities = [0];
    const scanState: ScanState = { scanLeftToRight: true, scanBottomToTop: true, toggleScanDirection: false };
  ...
  it('property: scanState toggles correctly based on toggleScanDirection flag', () => {
    fc.assert(
      fc.property(fc.boolean(), fc.boolean(), fc.boolean(), (left, bottom, toggle) => {
        const custom: Dims = { width: 1, height: 1 };
        const grid = new Uint8Array([0]);
        const newGrid = new Uint8Array([0]);
        const cellMap: Record<number, CellDefinition> = {
          0: { priority: 0, update: () => {} },
        ...
        const priorities = [0];
        const scanState: ScanState = { scanLeftToRight: left, scanBottomToTop: bottom, toggleScanDirection: toggle };
