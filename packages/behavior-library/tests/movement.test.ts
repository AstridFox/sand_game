...
describe('movement behaviors', () => {
  ...
  test('createFall behaves like fall', () => {
    ...
    grid[index] = SAND
    newGrid.set(grid)
    const moved = behavior({} as unknown as Cell, index, grid, newGrid, dims)
    expect(moved).toBe(true)
    expect(newGrid[index + dims.width]).toBe(SAND)
  })
  ...
  test('fluid moves particle sideways when down blocked', () => {
    ...
    expect(moved).toBe(true)
    const leftMoved =
      newGrid[index - 1] === WATER || newGrid[index + 1] === WATER
  ...
  test('createFluid and createGas wrappers', () => {
    ...
    grid[idx] = WATER
    newGrid.set(grid)
    expect(fluidBehavior({} as unknown as Cell, idx, grid, newGrid, dims)).toBe(
      true,
    )
    grid[idx] = SAND
    newGrid.set(grid)
    expect(gasBehavior({} as unknown as Cell, idx, grid, newGrid, dims)).toBe(
      true,
    )
  })
})
