...
describe('interaction behaviors', () => {
  ...
  test('createRandomTransform with prob=1 always transforms', () => {
    ...
    const did = transform({} as unknown as Cell, center, grid, newGrid, dims)
  ...
  test('createNeighborTrigger triggers on adjacent cell', () => {
    ...
    const trigger = createNeighborTrigger({
      triggerCells: [FIRE],
      result: FIRE,
      prob: 1,
    })
    const did = trigger({} as unknown as Cell, center, grid, newGrid, dims)
    expect(did).toBe(true)
    expect(newGrid[center]).toBe(FIRE)
  })
  ...
  test('createNeighborPropagation propagates to neighbor', () => {
    grid[center - 1] = FIRE
    const propagate = createNeighborPropagation({
      triggerCells: [FIRE],
      prob: 1,
    })
    const did = propagate(
      { id: WATER } as unknown as Cell,
      center - 1,
      grid,
      newGrid,
      dims,
  ...
  test('createNeighborSchedule calls action when triggered', () => {
    ...
    grid[center - 1] = FIRE
    const schedule = createNeighborSchedule({
      triggerCells: [FIRE],
      result: FIRE,
      action,
    })
    const did = schedule({} as unknown as Cell, center, grid, newGrid, dims)
    expect(did).toBe(true)
