...
const pendingTrees: PendingTree[] = []
 ...
 */
export function scheduleTree(startX: number, startY: number, dims: Dims): void {
  ...
  const branches: { x: number; y: number; dir: number; depth: number }[] = []
 ...
 */
export function processGrowth(grid: Uint8Array, dims: Dims): void {
