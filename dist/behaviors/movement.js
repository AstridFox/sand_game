...
export function fall(index, grid, newGrid, dims, allowed) {
...
export function fluid(index, grid, newGrid, dims, allowed) {
    const { width, height } = dims;
    const y = Math.floor(index / width);
    const x = index % width;
    if (y < height - 1) {
...
export function createFall(opts) {
    return (cell, index, grid, newGrid, dims) => fall(index, grid, newGrid, dims, opts.allowed);
}
/** Factory for fluid behavior. */
export function createFluid(opts) {
    return (cell, index, grid, newGrid, dims) => fluid(index, grid, newGrid, dims, opts.allowed);
}
/** Factory for gas behavior. */
export function createGas(opts) {
    return (cell, index, grid, newGrid, dims) => gas(index, grid, newGrid, dims, opts.allowed);
}
